...
public class Dwelling{
	...
	public Dwelling(DwellingFloor[] arr_dwelling_floor){
		...
		for(int i=0; i < arr_dwelling_floor.length; i++){
			this.floors_array[i]= new DwellingFloor(arr_dwelling_floor[i]);
		}
	}
	...
	public setDwellingFloor(int number_of_dwelling_floor, DwellingFloor new_dwelling_floor){
		...
		{
			if(new_dwelling_floor.length==this.floors_array[number_of_dwelling_floor].length){
				for(int i =0; i < new_dwelling_floor.length; i++){
					this.floors_array[number_of_dwelling_floor].setFlat(new_dwelling_floor[i]);
				}
			}
		}	
	}
	...
	public Flat getFlatByNumber(int number_of_flat){
		if (number_of_flat<0 || number_of_flat>this.getNumbersOfFlats-1) System.out.println("getFlatByNumber: wrong number_of_flat");
		else{
			int[] flat_index=this.getFlatIndex(number_of_flat);
			return this.floors_array[flat_index[0]][flat_index[1]];
			}
	...
	public void setFlat(int number_of_flat, Flat new_flat){
		if (number_of_flat<0 || number_of_flat>this.getNumbersOfFlats()-1) System.out.println("setFlat: wrong number_of_flat");
		else{
			Flat flat=this.getFlatByNumber(number_of_flat);
			flat.setSquare(new_flat.getSquare());
			flat.serNumbersOfRooms(new_flat.getNumbersOfRooms());
	...
	public void addFlat(int number_of_new_flat, Flat new_flat) {
		if(number_of_new_flat<0 || number_of_new_flat> this.getTotalNumberOfFlats()-1) System.out.println("addFlat: wrong number_of_new_flat");
		else{
			int buff_flat_index=number_of_flat;
			for(int i =0; i < this.floors_array.length; i++){
				if (buff_flat_index<=this.floors_array[i].getNumbersOfFlats()-1){
	...
	public Flat[] getSortArray(){
		...
		for(DwellingFloor floor : this.floors_array){
			for(Flat flat : floor){
				sort_flats_array[i]=new Flat(flat.getNumbersOfRooms(),flat.getSquare());
			}
		}
		//сама сортировка
		for(int i=0; i < sort_flats_array.length-1; i++){
			for (int j = 1; j< sort_flats_array.length; j++){
				if (sort_flats_array[i]<sort_flats_array[j]){
